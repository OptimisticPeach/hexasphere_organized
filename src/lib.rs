...
impl Chunked {
    ...
    pub fn blob_borders(self, blob: &Blob) -> Option<Vec<Vec<Coordinate>>> {
        self
            .find_blobs(blob.borders.iter().copied())
    ...
    pub fn ring_order(self, coordinates: &mut [Coordinate], inside: &HashSet<Coordinate>) {
        ...
        let common_out = self.surrounding(second)
            .into_iter()
            .filter(|x| first_surrounding.contains(x) && !inside.contains(x))
            .next()
            .unwrap();

        // Since its outside, the first two should have reversed winding for it.
                ...
                (0..5u8)
                    .flat_map(move |chunk|
                        (0..self.subdivisions + 1)
                            .flat_map(move |short|
                                (0..(self.subdivisions + 1) * 2)
                                    .map(move |long| Coordinate::Inside { chunk, short, long })
                            )
                    )
            )
    }
...
impl<T> Hexasphere<T> {
    ...
    pub fn get_many_mut<const N: usize>(&mut self, coordinates: [Coordinate; N]) -> Option<[&mut T; N]> {
        ...
        let coords = [c0, c1, c2, c3, c4].map(|x| (&mut **x).as_mut_ptr());
...
impl Debug for Coordinate {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Coordinate::Inside { chunk, short, long } => write!(f, "I({}, {}, {})", chunk, short, long),
        }
    }
}
...
impl Ord for Coordinate {
    fn cmp(&self, other: &Self) -> Ordering {
        ...
        match (self, other) {
            ...
            (
                Inside { chunk: c1, short: s1, long: l1 },
                Inside { chunk: c2, short: s2, long: l2 },
            ) => {
                match c1.cmp(c2) {
                    Ordering::Equal =>
                        match s1.cmp(s2) {
